...
AICB-P2T3 · Ngày 24 · VinUniversity · Tháng 5, 2026

---

## Mục Lục

- [Chào mừng đến với Lab 24!](#chào-mừng-đến-với-lab-24)
- [Phần 1 — Thông tin chung](#phần-1--thông-tin-chung)
- [Phần 2 — Cấu trúc Lab](#phần-2--cấu-trúc-lab)
- [Phần 3 — Phase A: RAGAS Evaluation](#phần-3--phase-a-ragas-evaluation-60-phút-30-điểm)
- [Phần 4 — Phase B: LLM-as-Judge & Calibration](#phần-4--phase-b-llm-as-judge--calibration-60-phút-25-điểm)
- [Phần 5 — Phase C: Guardrails Stack](#phần-5--phase-c-guardrails-stack-90-phút-35-điểm)
- [Phần 6 — Phase D: Blueprint Document](#phần-6--phase-d-blueprint-document-30-phút-10-điểm)
- [Phần 7 — Submission](#phần-7--submission)
- [Phần 8 — Self-Assessment Checklist](#phần-8--self-assessment-checklist)
- [Phần 9 — Bonus Points](#phần-9--bonus-points-tối-đa-15)
- [Phần 10 — FAQ](#phần-10--faq)
- [Phần 11 — Quick Reference](#phần-11--quick-reference)
...
| Deliverable | GitHub repo + Blueprint document |
...
df = pd.read_csv('pairwise_results.csv').sample(10, random_state=42)
df[['question', 'answer_a', 'answer_b']].to_csv('to_label.csv', index=False)
```

**Bước 2:** Mở `to_label.csv`, tự đọc và judge 10 cặp. Save thành `human_labels.csv`:
...
Implement topic validator với 1 trong 3 cách. Chọn 1 option phù hợp với skill level:
...
Dùng Groq API (free tier hỗ trợ Llama Guard 3). Đăng ký groq.com, lấy API key, dùng Option B trong Task C.4.

**Q2: RAGAS chạy quá lâu (>10 phút cho 50 questions)?**
Vấn đề rate limit. Giảm `max_concurrent` xuống 2, hoặc switch sang `gpt-4o-mini` (rẻ + nhanh hơn 5x so với `gpt-4o`).

**Q3: Cohen's kappa = -0.1, tôi sai gì?**
Có thể bạn label winner theo column position thay vì content. Re-check: human label có thực sự dựa trên answer content không? Hoặc human + judge label inconsistent (1 dùng "A", 1 dùng "answer_a") — normalize labels trước khi compute.

**Q4: Test set generation gen ra questions kỳ lạ (không liên quan domain)?**
Đây là vấn đề thường gặp. 3 fixes:
...
Có thể bạn run sequentially. Verify:
...
4-6 trang là target. Quality > length. Một blueprint 3 trang với diagram tốt và analysis sâu > 8 trang generic.

**Q7: Tôi có thể submit muộn không?**
Default policy: -10% mỗi ngày muộn. Tối đa 3 ngày. Sau đó: 0 điểm.

**Q8: Có thể dùng AI assistant (Claude, Copilot) không?**
Có — đây là course "vibe coding". Phải:
...
OK, miễn là nó chạy được. Lab này focus vào eval + guardrail, không phải RAG pipeline. Đơn giản còn dễ identify failure modes hơn.

**Q10: Tôi không tìm được unsafe outputs để test Llama Guard?**
Manually craft 10 unsafe responses theo template:
